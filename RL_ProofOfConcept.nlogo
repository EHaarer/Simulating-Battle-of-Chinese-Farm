...
turtles-own [
  state           ;; (x, y) for Q-learning
  action          ;; chosen movement action
  team            ;; "israeli" or "egyptian"
  group-id        ;; group identifier for cohesion
  canal-wait      ;; how many ticks left to wait on the canal
]
...
to setup
  ...
  ;; Make a 100x100 world: x = 0..99, y = 0..99
  resize-world 0 (battlefield-width - 1) 0 (battlefield-height - 1)
  set-patch-size 5

  ;; If needed, uncheck wrapping in the GUI’s Settings.

  set canal-x floor (battlefield-width / 2)  ;; e.g., 50 if width=100
...
to q-learn-move-israeli
  ...
  [
    execute-action a
    ;; Group cohesion: distance <= 6
    if distance my-group-center > 6 [
      setxy oldx oldy
    ]
  ]

  ;; Possibly kill adjacent Egyptians (no kill reward for Israelis)
...
to-report compute-reward [s s2]
  ...
  ;; If we stand on Chinese Farm, +200
  if member? patch-here chinese-farm-patches [
    set reward (reward + 200)
  ]

  ;; -20 if we didn't move
  if ((item 0 s) = (item 0 s2)) and ((item 1 s) = (item 1 s2)) [
    set reward (reward - 20)
  ]

  ;; PARTIAL REWARD for stepping onto canal columns
  ;; from east side x >= canal-x+2 into canal-x-1..canal-x+1
  if (team = "israeli" and oldx >= (canal-x + 2)
      and newx >= (canal-x - 1) and newx <= (canal-x + 1)) [
    set reward (reward + 200)
  ]

  ;; BIG REWARD for fully crossing from x >= canal-x+2 to x <= canal-x-2
